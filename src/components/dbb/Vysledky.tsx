    ...
    <Okno titulek={t("Moje výsledky", "My Results")} zavrit={zavrit} sirka={560}>
      <div className="px-5 py-4 text-[12px] leading-relaxed">
        ...
        <p className="mt-2 text-dbb-slaby">
          <span className="mr-1 inline-block h-2.5 w-2.5 bg-[#2e9d4f] align-middle" /> {t("splněno samostatně", "completed on your own")}
          <span className="ml-3 mr-1 inline-block h-2.5 w-2.5 bg-[#c8c8c8] align-middle" />{" "}
          {t("šedá = splněno s pomocí tlačítka Vložit do editoru", "grey = completed with the Insert into Editor button")}
        </p>
        <p className="mt-1 text-dbb-slaby">
          {t("Vložená řešení", "Inserted solutions")}: {vlozeno} · {t("úlohy navíc", "extra tasks")}: {p.navic}
