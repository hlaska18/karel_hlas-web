...
import { Search, Download, GraduationCap, Users, X } from "lucide-react";
          ...
          return (
            <li key={it.href}>
              <a
                href={it.href}
                download
                className="glass group flex items-center gap-4 rounded-2xl px-4 py-3.5 transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-accent-600/15"
              >
                ...
                <span className="hidden w-16 shrink-0 text-right text-xs text-zinc-400 md:block">
                  {fmtSize(it.sizeBytes)}
                </span>
                <Download className="h-5 w-5 shrink-0 text-zinc-400 transition group-hover:text-accent-600 dark:group-hover:text-accent-400" />
              </a>
            </li>
          );
        })}
