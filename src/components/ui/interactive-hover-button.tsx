 ...
 *  - `size`: "md" (výchozí, úvodní sekce) | "sm" (kompaktní, horní menu)
 */
...
type CommonProps = {
  ...
  icon?: ReactNode;
  /** Velikost: "md" (úvod) nebo "sm" (hlavička). Výchozí "md". */
  size?: Size;
  className?: string;
};
...
const SIZE: Record<Size, { box: string; dot: string }> = {
  md: { box: "min-h-[44px] py-3 pl-10 pr-6 text-sm", dot: "left-4 h-2 w-2" },
  sm: { box: "min-h-[40px] py-2 pl-8 pr-5 text-[0.8125rem]", dot: "left-3 h-1.5 w-1.5" },
};
...
export function InteractiveHoverButton(props: AnchorProps | NativeButtonProps) {
  const { text, icon, size = "md", className = "" } = props;
  const s = SIZE[size];
  const cls = `${baseCls} ${s.box} ${className}`.trim();

  if (props.href !== undefined) {
    const { text: _t, icon: _i, size: _s, className: _c, ...rest } = props as AnchorProps;
    return (
      <a {...rest} className={cls}>
        <Inner text={text} icon={icon} dot={s.dot} />
      </a>
    );
  }

  const { text: _t, icon: _i, size: _s, className: _c, ...rest } = props as NativeButtonProps;
  return (
