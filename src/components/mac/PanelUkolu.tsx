...
/** Barvy oken Moje výsledky a Přehled třídy – ať vypadají jako Mac. */
const VZHLED: Vzhled = {
  okno: "rounded-xl bg-mac-povrch shadow-[var(--mac-stin)] mac-bezvyberu",
  text: "text-mac-text",
  slaby: "text-mac-slaby",
  linka: "border-mac-linka",
  najeti: "hover:bg-mac-zvyrazneny",
  akcent: "bg-mac-akcent",
  akcentText: "text-mac-akcent-text",
  akcentPismo: "text-mac-akcent",
  pole: "border border-mac-linka bg-mac-povrch text-mac-text",
};
  ...
  return (
    <>
      ...
      >
        ...
        <footer className="flex shrink-0 items-center gap-3 border-t border-mac-linka px-4 py-2.5 text-[12px]">
          <button
            type="button"
            onClick={() => nastavOkno("vysledky")}
            className="rounded-md bg-mac-akcent px-3 py-1.5 font-semibold text-mac-akcent-text hover:opacity-90"
          >
            Moje výsledky
          </button>
          <button type="button" onClick={() => nastavOkno("prehled")} className="text-[11px] text-mac-akcent hover:underline">
