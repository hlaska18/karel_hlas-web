...
/** Pořadí typů souborů pro ukázku – ať je na kartě něco názorného, ne `.db`. */
const SHOWCASE_EXT = ["docx", "pdf", "py", "sql", "xlsx", "pbix", "txt"];

const showcaseRank = (ext: string) => {
  const i = SHOWCASE_EXT.indexOf(ext);
  return i === -1 ? SHOWCASE_EXT.length : i;
};

/**
 * Pár skutečných materiálů jako ukázka v heru. Vybírá se z každého nástroje
 * jeden (v pořadí dlaždic) a pak se preferují RŮZNÉ typy souborů, ať stoh
 * karet nevypadá jako třikrát totéž. Učitelské materiály se nenabízejí.
 */
export function getHeroHighlights(items: BankItem[], count = 3): BankItem[] {
  const pool = items.filter((i) => !i.external && i.audience !== "teacher");

  // Kandidáti za každý nástroj (v pořadí dlaždic galerie), od nejnázornějšího.
  const perTool = TOOL_ORDER.map((tool) =>
    pool
      .filter((i) => i.tool === tool)
      .sort((a, b) => showcaseRank(a.ext) - showcaseRank(b.ext) || byName(a.label.cs, b.label.cs)),
  ).filter((list) => list.length > 0);

  // Z každého nástroje nejvýš jeden. 1. průchod bere jen dosud nepoužité
  // přípony (i za cenu horšího kandidáta), 2. doplní zbytek do počtu.
  ...
  for (const onlyNewExt of [true, false]) {
    for (const list of perTool) {
      ...
      if (usedTools.has(list[0].tool)) continue;
      const it = onlyNewExt ? list.find((i) => !usedExt.has(i.ext)) : list[0];
      if (!it) continue;
      picked.push(it);
      usedTools.add(it.tool);
      usedExt.add(it.ext);
    }
  }
